...
export default function CategoryPage() {
  ...
  const addNewCategory = async (name: string): Promise<boolean> => {
    ...
    try {
      ...
      setCategories((prev) => [...prev, { id: docRef.id, name: originalName }]);
      setName("");
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Kelola semua kategori hasil tani</p>
        </div>
      ...
      {/* List Kategori */}
      <CategoryList categories={categories} onDeleteCat={handleDeleteCategory} />
      ...
      {categoriesToDelete && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-md w-80">
            ...
            <p>
              Yakin ingin menghapus kategori <strong>{categoriesToDelete.name}</strong>?
            </p>
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <button
                onClick={async () => {
                  try {
                    await deleteDoc(doc(db, "category", categoriesToDelete.id));
                    setCategories(categories.filter((c) => c.id !== categoriesToDelete.id));
